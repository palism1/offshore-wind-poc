...
7. `simulator`: the rolling-window loop orchestrating 1 through 6 day by day until the stress window ends. Has a terminal entry point: `owr.cli` (console script `simulate`, also runnable as `python -m owr`), which reads a day-profile CSV and drives this loop end to end. See `docs/archive/plans/PLAN_SIMULATOR_CLI.md`.
