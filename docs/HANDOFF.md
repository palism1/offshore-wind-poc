...
1. The project became a **git repo** during this work (`git init`, then pushed to a
   private GitHub remote). Because of that, Claude Code background isolation now triggers:
...
2. There is a `.claude/settings.json` with `worktree.bgIsolation: none` intended to
   disable that guard, but it did **not** take effect mid session. Do not rely on it.
3. `.claude/` (worktrees, per repo settings) is gitignored.
...
5. Fresh sessions must invoke the personal skill **`fact-checking-grid-architecture`**
