...
# run a scenario end to end from the terminal — no database or credentials required
...
examples/           synthetic day-profile CSV for `simulate --input`, and the
                     generator script that produces it (see below)
docker-compose.yml  Postgres 16 + TimescaleDB for local dev
```

`examples/synthetic_winter_stress.csv` is generated by
`examples/make_synthetic_winter_stress.py`, not hand-written or pulled from ISO-NE.
Its column layout (`date,hour,load_mw,wind_mw`) is provisional: nothing in
`db/migrations/` or the Phase 2 ETL plan defines a matching export yet, so the
simulator CLI's day-profile reader and a future ETL export will need to be
reconciled when Phase 2 lands.

`examples/real_winter_stress_2026.csv` holds real ISO-NE load for the 2026-01-24 to
2026-02-03 stress event, generated by `etl demo-profile` (`docs/PLAN_REAL_DEMO_BRIDGE.md`);
it carries a real EIA-930 hourly wind column and no `wind_forecast_frac`
column, and needs regeneration from `data/` when the day-profile format changes.
